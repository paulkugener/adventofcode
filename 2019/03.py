...
U62,R66,U55,R34,D71,R55,D58,R83""" # assert = 159
example2 = """R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51
U98,R91,D20,R16,D67,R40,U7,R15,U6,R7""" # assert = 135
...
def run1(p):
    ...
    intersection_list = wire_list[0] & wire_list[1]
    manhattan_distance_list = list()
    for intersection in intersection_list:
        #print(intersection, abs(intersection[0]+intersection[1]))
        manhattan_distance_list.append(abs(intersection[0]) + abs(intersection[1]))
    return min(manhattan_distance_list)

assert run1(example1) == 159
assert run1(example2) == 135
    ...
    for line in p.splitlines():
        ...
        for op in ops:
            ...
            for _ in range(distance):
                ...
                wire[(x, y)] = step
        wire_list.append(wire)
    intersection_steps_list = set(wire_list[0].keys()) & set(wire_list[1].keys())
    return min((wire_list[0].get(i) + wire_list[1].get(i)) for i in intersection_steps_list)

assert run2(example1) == 610
assert run2(example2) == 410
